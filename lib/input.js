...
function signOut(message) {
  ...
  fs.unlink('journal.txt', function (err) {
    if(err) throw err;
  })
...
// creates text file containing default journal

function saveDefaultJournal(journal = 'Work Journal') {
  fs.writeFile('./journal.txt', journal, function (err) {
    if (err) throw err;
  })
  saveJournalApi(journal);
}
...
function saveJournalApi(journal) {
  superagent
    ...
    .set('Authorization', jwt)
    .send({ userId: userId, name: journal })
    .then(res => console.log('im in savejournalapi', res.body))
    .catch(err => console.error(err))
  // getJournals();

  // exitHandler(`Your default journal has been switched to ${journal}`)
}

// Queries API for list of journal entries associated with user
