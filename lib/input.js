...
function signOut(message) {
  fs.unlink('jwt.txt', function (err) {
    ...
    console.log('Please type in clij and sign-in or sign-up to continue.')
  })
}
...
function deleteEntry(id) {
  superagent
    ...
    .set('Authorization', jwt)
    .send(id)
    .then(res => console.log(`Entry ${id.id} successfully deleted.`))
    .catch(err => console.error('error', err))
}
...
function storeToken(token) {
  fs.writeFile('./jwt.txt', token, function (err) {
    ...
    console.log('Type in clij to continue')
  })
}
      ...
      .then(answer => {
        ...
        superagent
          ...
          .then(res => {
            ...
            console.log('Type in clij to start your journey.')
            storeToken(res.body.token)
          })
          .catch(err => console.error('error', err))
      });
