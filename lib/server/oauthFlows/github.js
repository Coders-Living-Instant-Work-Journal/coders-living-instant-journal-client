    ...
    createApplication(({ app, callbackUrl, server }) => {
      ...
      app.get('/gitHubOauth', async (req, res) => {
        ...
        try {
          ...
          const { email, id, login, name } = response.body;
          console.log('(3)--- User Data: ', { email, id, login, name });
          ...
          res.status(200).send('You logged in! You can now close this window.');
          resolve();
